...
- `keep_alive` (0 = unload the model after each image), max image side in px, CPU mode, **max tokens** cap (a rambling model is cut instead of hanging), **disable thinking** switch (default on: `think=false` is sent to models that declare the thinking capability, `--think` in the CLI re-enables it)
